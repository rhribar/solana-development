...
export async function fetchSwapState(swapStatePubkeyString: string): Promise<TradeState> {
    ...
    const des = deserializeUnchecked(SWAP_STATE_SCHEMA, TradeState, account?.data as Buffer)
...
export async function transfer(isMaker: boolean, tradeState: TradeState, tradeAccountPubkeyStr: string) {
    const tx = new Transaction();
    const signers = [];
